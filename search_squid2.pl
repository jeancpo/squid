...
print OUT "<table width=\"100%\">\n<tr><th colspan=\"5\" class=\"H25H\">REPORTE PROXY</th></tr>\n";
print OUT "<tr><th class=\"H25L\">FECHA</th><th class=\"H25L\">DIRECCION IP</th><th class=\"H25L\">USUARIO</th><th class=\"H25L\">URL</th><th class=\"H25L\">ACCESO</th></tr>\n"; 
...
open (IN, "<$Logfile") or die ("No se Puede Abrir el Archivo.");		
	while (my $line = <IN>) 
	{
		...
		($unix_timestamp, $peso, $ip, $code, $value, $cmd, $url, $users, $destino, $type) = split(' ',$line);
			...
			#print "Fecha: ".$dia."/".$meses[$mes]."/".($year+1900)." ".$hora.":".$min.":".$sec." Ip: ".$ip." Usuario: ".$users."\n";
        		my $fecha=$dia."/".$meses[$mes]."/".($year+1900)." ".$hora.":".$min.":".$sec;
			if ($usuario eq $users){
			if ($users eq ''){$users = 'N/A';}			
			print OUT "<tr><td class=$fontColor>$fecha</td><td class=$fontColor>$ip</td><td class=$fontColor>$users</td><td class=$fontColor>$url</td><td class=$fontColor>$code</td></tr>\n";		
			}
			elsif ($usuario eq ''){
			if ($users eq ''){$users = 'N/A';}
			print OUT "<tr><td class=$fontColor>$fecha</td><td class=$fontColor>$ip</td><td class=$fontColor>$users</td><td class=$fontColor>$url</td><td class=$fontColor>$code</td></tr>\n";
